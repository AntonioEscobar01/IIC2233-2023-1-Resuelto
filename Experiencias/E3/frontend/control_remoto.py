...
class VentanaControlRemoto(QWidget):
    ...
    def generar_botones(self):
        ...
        self.on_off = QPushButton('OFF', self)  # COMPLETAR
        ...
        self.numeros = []
        for i in range(1, 10):
            self.numeros.append(QPushButton(str(i), self))
        # COMPLETAR
    ...
    def actualizar_canal(self):
        sender = self.sender()
        accion = sender.text()
        self.senal_canal.emit(str(accion))

    def actualizar_volumen(self):
        sender = self.sender()
        accion = sender.text()
        self.senal_volumen.emit(str(accion))
